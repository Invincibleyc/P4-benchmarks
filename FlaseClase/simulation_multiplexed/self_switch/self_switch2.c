...
int main(){
    ...
    /*  Initialization - part I: Detect when we switch to color on '0'. */
    while(s == INIT){
        sleep(1);
        HISTORY_COUNTERS
        RESET_COUNTERS
    ...
    /* Initialization - part II: Detect when we switch to count '1' */
    while(s == UP){
        sleep(1);
        HISTORY_COUNTERS
        RESET_COUNTERS
...
/* This function wrap a bash script to address the counters, extract a string 
 * with the 4 numbers in a row (separated by commas) and feed it to another
 * function (extract_number) for parsing. It update the counter array with
 * new values from the counters. */
...
/* This function gets a string with 4 numbers in it seperated by commas and
 * extract them to 4 unsigned int *array input. */
...
void delta_ts_handler(unsigned long ts0, unsigned long ts1){
    ...
    /*if( tmp != histo ){  */
	    ...
	    tm = *localtime(&mytime);
	    printf("%02d:%02d:%02d --> ", tm.tm_hour, tm.tm_min, tm.tm_sec);
	    printf("Cycle delay is: %.6Lf seconds.\n", tmp); 
	    histo = tmp;
