...
int main(){
	...
	if (pthread_create(&p0, NULL, delay, NULL)) { printf("pthread create error - p0.\n");}
	if (pthread_create(&p1, NULL, loss, NULL)) { printf("pthread create error - p1.\n");}

	if (pthread_join(p0, NULL)) { printf("pthread join failed - p0.\n");}
	if (pthread_join(p1, NULL)) { printf("pthread join failed - p1.\n");}
