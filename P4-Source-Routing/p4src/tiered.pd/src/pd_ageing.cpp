...
p4_pd_status_t p4_pd_prog_ageing_set_cb(int dev_id, int table_id,
					 p4_pd_notify_timeout_cb cb_fn,
					 void *cb_cookie) {
...
p4_pd_status_t p4_pd_prog_ageing_new_device(int dev_id) {
  device_state[dev_id] = new AgeingState();
  return 0;
}

p4_pd_status_t p4_pd_prog_ageing_remove_device(int dev_id) {
  assert(device_state[dev_id]);
  delete device_state[dev_id];
  return 0;
}

void p4_pd_prog_ageing_notification_cb(const char *hdr, const char *data) {
