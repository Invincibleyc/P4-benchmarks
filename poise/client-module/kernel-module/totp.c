...
static uint8_t	_timezone;
static uint32_t	_timeStep;

static int32_t TimeStruct2Timestamp(struct tm *time){
	unsigned long current_sec = mktime(time->tm_year + 1900, time->tm_mon + 1, time->tm_mday,
			time->tm_hour, time->tm_min, time->tm_sec);
	return current_sec - (_timezone * 3600) - 2208988800;
}
...
uint32_t totp_getCodeFromSteps(uint32_t steps) {
	...
	sha1_initHmac(_hmacKey, _keyLength);
	sha1_writeArray(_byteArray, 8);
...
uint32_t totp_getCodeFromTimeStruct(struct tm time) {
	return totp_getCodeFromTimestamp(TimeStruct2Timestamp(&time));
}
...
uint32_t totp_getCodeFromTimestamp(uint32_t timeStamp) {
	...
	return totp_getCodeFromSteps(steps);
}
